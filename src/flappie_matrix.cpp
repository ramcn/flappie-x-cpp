...
flappie_matrix make_flappie_matrix(size_t nr, size_t nc) {
    ...
    //if (((void **)&mat->data.v, 16, nrq * nc * sizeof(__m128))) {
    mat->data.v = malloc(nrq * nc * sizeof(__m128));
    //fprintf(stderr,"calling vio malloc for size %lu\n",nrq*nc); 
    //mat->vio   = (float *)vio_malloc(nrq * nc);
...
flappie_matrix free_flappie_matrix(flappie_matrix mat) {
    if (NULL != mat) {
        //fprintf(stderr,"calling vio free for size %lu\n",mat->nrq*mat->nc); 
        free(mat->data.v);
        //vio_free(mat->vio);
        free(mat);
    }
...
flappie_imatrix make_flappie_imatrix(size_t nr, size_t nc) {
    ...
    mat->data.v = malloc(nrq * nc * sizeof(__m128i));
    if (mat->data.v == NULL) {
        warnx("Error allocating memory in %s.\n", __func__);
        free(mat);
        return NULL;
