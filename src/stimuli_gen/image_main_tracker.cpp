...
// Read mask points binary file
    ...
    print_config(CONFIG, width, height, threshold, tthreshold, num_points, v.size(), v.size(), 1, 1, num_points, num_points, 8, 8, 8, 8, 8, 8, 0, 20, 256, 1, 1, 2);
    ...
    Mat rotate_mat, finalimg, trans_mat, transimg;
#define angle_max 10
    float angle_trk = angle_max*((float)(rand() % angle_max)/(float)angle_max -0.5);//-MAX/2 to +MAX/2
    //float angle_trk = 0.000000;
    int rotate_center_x = copyimg.cols/2;
    int rotate_center_y = copyimg.rows/2;
    double scale = 1.0;
    fprintf(SEED, "angle: %f, x_center: %d, y_center: %d\n", angle_trk, rotate_center_x, rotate_center_y);
    printf("angle: %f, x_center: %d, y_center: %d\n", angle_trk, rotate_center_x, rotate_center_y);
    Point center = Point(rotate_center_x, rotate_center_y);
   
#if 0
    trans_mat = (Mat_<double>(2,3) << 1, 0, offsetx, 0, 1, offsety);
    warpAffine(copyimg, transimg, trans_mat, copyimg.size());
#endif 


    
    rotate_mat = getRotationMatrix2D( center, angle_trk, scale);

    warpAffine(copyimg, finalimg, rotate_mat, copyimg.size());
    imwrite("finalimg.pgm", finalimg);

    finalimg_data = (unsigned char*)finalimg.data;
    if (trkcbbuffassignment == 0 || trkcbbuffassignment == 1){
...
#define PI 3.14159265
                ...
                float curr_x = (((v[fid].x * cos(-angle_trk * PI/180.0)) -((cos(-angle_trk * PI/180.0)) * (rotate_center_x))) - ((v[fid].y * sin(-angle_trk * PI/180.0)) - (sin(-angle_trk * PI/180.0) * (rotate_center_y))) + (rotate_center_x));
                pred.x = (int)(curr_x + 0.5f);
                
                float curr_y = (((v[fid].x * sin(-angle_trk * PI/180.0)) -((sin(-angle_trk * PI/180.0)) * (rotate_center_x))) + ((v[fid].y * cos(-angle_trk * PI/180.0)) - (cos(-angle_trk * PI/180.0) * (rotate_center_y))) + (rotate_center_y));
...
#if SORT_INS
    g_fp_sort_1 = fopen((instr_dir + "__Sort1.txt").c_str(), "w+"); assert(g_fp_sort_1);
...
#if SORT_INS
    g_fp_sort_1 = fopen((instr_dir + "__Sort2.txt").c_str(), "w+"); assert(g_fp_sort_1);
...
// Read mask points binary file
    ...
    mask_1.compress();
    if (trkcbbuffassignment == 0 || trkcbbuffassignment == 1){
        dump_mem2d((instr_dir + "__Det1_Mask.txt").c_str(), mask_1.compressed,
                   mask_1.compressed_width, mask_1.scaled_height, mask_1.compressed_width);
    } else {
        dump_mem2d((instr_dir + "__Det2_Mask.txt").c_str(), mask_1.compressed,
                   mask_1.compressed_width, mask_1.scaled_height, mask_1.compressed_width);
    }
#else
    ...
    print_config(CONFIG, width, height, threshold, tthreshold, num_points, det1_points, det2_points, 1, 1, pred_list.size(), pred_list.size(), 8, 8, 8, 8, 8, 8, 0, mipirate, detworkset, trkcbbuffassignment, cbdet1buffassignment, cbdet2buffassignment);
