...
// Read mask points binary file
    ...
    int detworksetmax = 255;
    int detworksetmin = 32;
    int detworkset = ((rand() % (detworksetmax - detworksetmin)) + detworksetmin);

    print_config(CONFIG, width, height, threshold, tthreshold, num_points, det1_points, det2_points, 1, 1, pred_list.size(),
                 pred_list.size(), 8, 8, 8, 8, 8, 8, 0, mipirate, detworkset, trkcbbuffassignment, cbdet1buffassignment, cbdet2buffassignment);
